...
async def gemini_verdict(req: GeminiVerdictRequest):
    ...
    user_prompt = (
        ...
        "5 paragrafe principale notate AA) BB) CC) DD) EE) (3-5 propozitii fiecare).\n"
        "Dupa fiecare paragraf scrie un bloc de validare externa cu tag de sentiment: [OBJ:POS]...[/OBJ] daca datele externe confirma pozitiv,"
        " [OBJ:NEG]...[/OBJ] daca contrazic sau sunt negative, [OBJ:NEU]...[/OBJ] daca sunt neutre sau insuficiente. 1-2 propozitii.\n"
        "La final un bloc [GENERAL]...[/GENERAL]. Fara titluri, fara liste, fara text in afara acestui format.\n"
        "\nAA) Evalueaza daca scorul matematic reflecta corect realitatea."
        " Citeaza numerele concrete (scor, marja, probabilitate). Spune daca esti de acord cu castigatorul sau nu.\n"
        "[OBJ:POS/NEG/NEU]Compara P/E si dividend yield al castigatorului cu media sectorului din datele Damodaran de mai sus. Citeaza numerele concrete.[/OBJ]\n"
        "\nBB) Compara asimetria P90 vs P10 pentru fiecare actiune."
        " Cine are cel mai bun raport potential/risc real, independent de P50?"
        " Cum se coreleaza volatilitatea cu asimetria?\n"
        "[OBJ:POS/NEG/NEU]Raporteaza volatilitatea la contextul sectorului. Este ridicata sau normala pentru sector? Citeaza benchmark-ul sectorial.[/OBJ]\n"
        "\nCC) Identifica principalele contradictii din date."
        " Marja de siguranta contrazice randamentul MC? Sentimentul negativ sau pozitiv schimba contextul?"
        " Dividendul este sustenabil?\n"
        "[OBJ:POS/NEG/NEU]Compara dividend yield-ul fiecarei actiuni cu media sectorului din datele de mai sus. Citeaza numerele si exprima o opinie clara.[/OBJ]\n"
        f"\nDD) {par4_label} —"
        f" {par4.split(chr(8212))[-1].strip() if chr(8212) in par4 else par4}\n"
        "[OBJ:POS/NEG/NEU]Pe baza recomandarilor analistilor si stirilor: consensul extern confirma sau contrazice simularea? Citeaza buy/hold/sell count.[/OBJ]\n"
        "\nEE) Recomanda o singura actiune pentru intrare acum cu un pret de intrare specific."
        " Numeste un catalyst concret asteptat (ex: rezultate Q, dividend, aprobare regulatorie) si un nivel stop-loss clar.\n"
        "[OBJ:POS/NEG/NEU]Ce date externe (sectorial, analisti, stiri) sustin sau slabesc aceasta recomandare? Fii specific si direct.[/OBJ]\n"
        "\n[GENERAL]Concluzie directa: care actiune ai alege tu personal si de ce, intr-o singura propozitie."
        " Apoi 2 propozitii despre contextul macro relevant pentru aceste companii.[/GENERAL]"
    )
...
async def groq_verdict(req: GroqVerdictRequest):
    ...
    user_prompt = (
        ...
        "\nAA) Evalueaza daca scorul matematic reflecta corect realitatea."
        " Citeaza numerele concrete (scor, marja, probabilitate). Spune daca esti de acord cu castigatorul sau nu.\n"
        "[OBJ:POS/NEG/NEU]Compara P/E si dividend yield al castigatorului cu media sectorului din datele Damodaran. Citeaza numerele.[/OBJ]\n"
        "\nBB) Compara asimetria P90 vs P10 pentru fiecare actiune."
        " Cine are cel mai bun raport potential/risc, independent de P50?\n"
        "[OBJ:POS/NEG/NEU]Raporteaza volatilitatea la contextul sectorului. Este ridicata sau normala? Citeaza benchmark-ul sectorial.[/OBJ]\n"
        "\nCC) Identifica principalele contradictii din date."
        " Marja contrazice randamentul MC? Dividendul este sustenabil?\n"
        "[OBJ:POS/NEG/NEU]Compara dividend yield-ul cu media sectorului. Citeaza numerele.[/OBJ]\n"
        f"\nDD) {par4_label} — {par4.split(chr(8212))[-1].strip() if chr(8212) in par4 else par4}\n"
        "[OBJ:POS/NEG/NEU]Consensul extern confirma sau contrazice simularea? Citeaza buy/hold/sell count.[/OBJ]\n"
        "\nEE) Recomanda o singura actiune pentru intrare acum cu un pret de intrare specific."
        " Numeste un catalyst concret asteptat si un nivel stop-loss clar.\n"
        "[OBJ:POS/NEG/NEU]Ce date externe sustin sau slabesc aceasta recomandare? Fii specific si direct.[/OBJ]\n"
        "\n[GENERAL]Concluzie directa: care actiune ai alege tu personal si de ce, intr-o singura propozitie."
        " Apoi 2 propozitii despre contextul macro relevant pentru aceste companii.[/GENERAL]"
    )
